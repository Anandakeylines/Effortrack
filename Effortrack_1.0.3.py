...
from PyQt5.QtWidgets import (
    ...
    QLineEdit, QMessageBox, QCheckBox, QHBoxLayout
)
...
def add_to_startup():
    ...
    if getattr(sys, 'frozen', False):
        # If bundled with PyInstaller (EXE)
...
def setup_logging():
    # Always use a user-writable log directory
    log_dir = QStandardPaths.writableLocation(QStandardPaths.AppDataLocation)
    if not log_dir:  # fallback in case Qt doesn't return one
...
#     try:
#         if idle_status==0:
#             print("[DEBUG] Sending idle status screenshot")
...
#                 }
#                 print(f"[DEBUG] Sending screenshot file: {file_path}")
...
class ScreenshotApp(QWidget):
    def __init__(self, config_manager):
        ...
        self.initUI()
        QTimer.singleShot(100, self.toggle_btn.click)#start btn automatacly call whenever app start
        set_device_type()
...
""")
    ...
    def update_idle_state(self, idle_time):
        ...
        if is_idle != self.was_idle or getattr(self, "_last_tray_state", None) != is_idle:
            if not is_idle and self.was_idle:
                # Small delay when going from idle → active
                time.sleep(1)
    ...
    def screenshot_loop(self):
        try:
            ...
            while self.screenshot_active and not self._shutting_down:
                try:
                    #  Lock while checking idle status (based on last user activity)
                    with self.idle_monitor.lock:
                        is_idle = (time.time() - self.idle_monitor.last_activity) >= self.idle_threshold
        
                    current_time = time.time()
    
                    #  1. IDLE MODE: Upload idle image once per interval
                    if is_idle:
                        if not idle_image_sent or (current_time - last_upload_time) >= self.screenshot_interval:
                            idle_file = resource_path("idle.jpg")  # static file bundled with your app
                            send_screenshot(USER_ID, ORG_ID, file_path=idle_file, idle_status=0)
                               
                            idle_image_sent = True
                            last_upload_time = current_time
    
                    #  2. SWITCH: From IDLE  ACTIVE → Take screenshot immediately
                        ...
                        send_screenshot(USER_ID, ORG_ID, file_path=final_file)
                        ...
                        last_upload_time = current_time
                        idle_image_sent = False  # Reset idle flag
    
                    #  3. ACTIVE MODE: Every 5 minutes, take screenshot
                    elif (current_time - last_upload_time) >= 300:  # 300 seconds
                        timestamp = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
                        final_file = f"screenshots/{USER_ID}_{ORG_ID}_{timestamp}.jpg"
                    ...
                    #  Update previous idle state for next loop
    ...
    def reset_idle_timer(self, *args):
        try:
            ...
            logger.error(f"Error in reset_idle_timer: {e}")
...
if __name__ == "__main__":
    ...
    splash.showMessage("V1.0.3", Qt.AlignBottom | Qt.AlignHCenter, Qt.black)
    app.processEvents()
    time.sleep(3)
