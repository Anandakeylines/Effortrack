...
class IdleMonitor:
    ...
    def stop(self):
        ...
        if self.thread.is_alive():
            self.thread.join(timeout=2)   # wait longer, more reliable
...
""")
    ...
    def closeEvent(self, event):
        """Keep the app in taskbar and also in tray when X is clicked."""
        event.ignore()  # prevent full quit
        self.showMinimized()  # minimize to taskbar (not hide completely)
        self.hide()
        self.tray_icon.showMessage(
            "Effortrak",
            "Application is still running (minimized to taskbar and tray)",
...
if __name__ == "__main__":
    ...
    splash.showMessage("V1.0.4", Qt.AlignBottom | Qt.AlignHCenter, Qt.black)
    app.processEvents()
    time.sleep(3)
